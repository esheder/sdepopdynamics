...
def kolmogorov(i, a1, a2, b1, b2, t, p0: int, nmax: int = 200):
    popsize = np.arange(nmax)
    ...
    psol = sol.y
    m1 = np.sum(popsize[:, np.newaxis] * psol, axis=0)
    x2 = np.sum((popsize[:, np.newaxis] ** 2) * psol, axis=0)
    x3 = np.sum((popsize[:, np.newaxis] ** 3) * psol, axis=0)
    m2 = x2 - m1 ** 2
...
def test_kolmogorov_no_change():
    moo = kolmogorov(0, 0, 0, 0, 0, np.linspace(0, 1, 20), 5, 8)
